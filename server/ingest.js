/* server/ingest.js - TF-IDF ingest */
import fs from "fs";
import path from "path";
function tokenize(text){ return (text||"").toLowerCase().replace(/[^a-z0-9\s]/g," ").split(/\s+/).filter(Boolean); }
function buildTf(tokens){ const tf={}; for(const t of tokens) tf[t]=(tf[t]||0)+1; const len = tokens.length||1; for(const k of Object.keys(tf)) tf[k]=tf[k]/len; return tf; }
async function run(){
  const docsDir = path.resolve("./ingest_data");
  if(!fs.existsSync(docsDir)){ console.error("ingest_data not found:", docsDir); return; }
  const files = fs.readdirSync(docsDir).filter(f=>f.endsWith(".txt")||f.endsWith(".md"));
  if(files.length===0){ console.log("No .txt/.md in ingest_data"); return; }
  const docs=[]; const docFreq={};
  for(const f of files){
    ...
    if(words.length===0){ docs.push({ id:`${f}#0`, text:"", tokens:[], tf:{} }); }
  }
  const N = docs.length;
  const idf = {};
  for(const [tok,df] of Object.entries(docFreq)){ idf[tok] = Math.log(1 + (N / (1 + df))); }
  const out = docs.map(d => {
    const tfidf = {};
    for(const [tok,tfv] of Object.entries(d.tf||{})){ tfidf[tok] = tfv * (idf[tok] ?? Math.log(1+N)); }
